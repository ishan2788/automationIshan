...
from iOS.IBS.functions_iRD import go_next, send_keys, click_element, swipe_down, scroll_right, newfinder
...
class Questionnaire:
    ...
    def test_questionnaire(self):
        ...
        path = '(//XCUIElementTypeOther[@name="Sehr schwierig"])[2]'
        next_path = '//XCUIElementTypeOther[@name="Weiter"]'
        newfinder(self.driver, path, next_path)

        for x in range(5):
            send_keys(self.driver, index=2, path="XCUIElementTypeTextField", keys="10")
            go_next(self.driver, case=0)
            time.sleep(4)
        ...
        click_element(self.driver, index=0, path='clinical_trial_onboarding_continue_button')
